...
# Gerar média e desvio padrão
rm -rf ../Saidas/med_desv
mkdir -p ../Saidas/med_desv

for entr in 20 100 250
do
	for cs in `seq 0 9`
	do
		python3 med_desvio.py < ../Saidas/bp_data/sa_${entr}_cs${cs}.dat > ../Saidas/med_desv/sa_${entr}_cs${cs}.dat
		python3 med_desvio.py < ../Saidas/bp_data/rs_${entr}.dat > ../Saidas/med_desv/rs_${entr}.dat
	done
done
